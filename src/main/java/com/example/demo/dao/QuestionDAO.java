...
public interface QuestionDAO extends JpaRepository<Question,Integer> {
	...
	Page<Question> getActivatedQuestions(String status,Pageable paging);
    
    @Query(value = "select * from questions where status = ?1",nativeQuery = true)
   	Page<Question> getDeactivatedQuestions(String status,Pageable paging);
    
    @Transactional
    @Modifying
    @Query(value = "update questions set status=?1 where id in(?2)",nativeQuery = true)
   	void updateStatus(String status,List<Integer> id);
    
    @Query(value = "SELECT * FROM questions WHERE FIND_IN_SET(?1, tags) and status=?2",nativeQuery = true)
   	Page<Question> getQuestionBasedOnTags(String tagName,String status,Pageable paging);
